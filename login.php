...
// Vérification du formulaire
 ...
 if(isset($_POST['submit'])) // le formulaire vient d'être soumis
 {
    ...
    { 
        ...
        {
            $ChampsIncorrects=$ChampsIncorrects.'<li>login (8 caractères minimum composé de lettres, chiffres, underscores, accents, points et caractères dans l\'ensemble {!#$*%^&?.-})</li>';
            $ClassLogin='error';
        }
    }
    ...
    if(isset($_POST['naissance']) && (trim($_POST['naissance']) !=''))// naissance vide
    {
     ...
     { 
         ...
         if(!preg_match("/^\d+([^\W\d_]|[\s])+\.?(?:[- ]([^\W\d_]|[\s])+\.?)*$/i", $ville)) // nombre suivis d'espaces, lettres, tirets, points, accents
         {
